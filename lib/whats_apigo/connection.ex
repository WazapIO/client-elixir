...
defmodule WhatsAPIGo.Connection do
  @moduledoc """
  Handle Tesla connections for WhatsAPIGo.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, WhatsAPIGo.Connection,
        base_url: "/api",
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:whatsapi, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - WhatsAPIGo 2.0 - elixir"
        )
      )
